...
def run_tracking():
    ...
    while tracking_active and not stop_event.is_set():
        ...
        if abs(offset_yaw) < DEAD_ZONE:
            ...
            offset_yaw = offset_yaw - DEAD_ZONE if offset_yaw > 0 else offset_yaw + DEAD_ZONE
        
        if abs(offset_pitch) < DEAD_ZONE:
            offset_pitch = 0
        else:
            offset_pitch = offset_pitch - DEAD_ZONE if offset_pitch > 0 else offset_pitch + DEAD_ZONE
        ...
        damp_x = 1.0 if closest_x > EDGE_MARGIN else MIN_DAMP + (1.0 - MIN_DAMP) * (closest_x / EDGE_MARGIN)
        damp_y = 1.0 if closest_y > EDGE_MARGIN else MIN_DAMP + (1.0 - MIN_DAMP) * (closest_y / EDGE_MARGIN)
...
def handle(cmd, core):
    ...
    # Start tracking
    if any(w in cmd_lower for w in ["start tracking", "begin tracking", "enable tracking"]):
    ...
    if any(w in cmd_lower for w in [
        "stop tracking", "end tracking", "close tracking", "quit tracking",
        "disable tracking", "tracking off", "stop track"
    ]):
...
def listen_for_tracking_commands(core):
    ...
    while tracking_active:
        ...
        cmd = core.transcribe(audio, prompt="click double click right click freeze go nudge up down left right recalibrate stop tracking close cancel")
        if not cmd:
            continue
        ...
        # Exit commands
        if any(w in cmd_lower for w in ["stop tracking", "end tracking", "close tracking",
                                         "stop", "cancel", "escape", "exit", "quit", "done"]):
            frozen = False
            stop_tracking()
            core.speak("Stopped")
            return
        ...
        if frozen and any(w in cmd_lower for w in ["nudge", "move", "up", "down", "left", "right"]):
        ...
        if any(w in cmd_lower for w in ["click", "select", "press", "kick", "quick", "flick"]):
